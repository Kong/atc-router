...
mod tests {
    ...
    fn expr_var_name_and_ip() {
        let tests = vec![
            ...
            // ipv6_cidr_literal
            ("kong.foo.foo4 in 2001:db8::/32","(kong.foo.foo4 in 2001:db8::/32)"),
        ];
        for (input, expected) in tests {
            let result = parse(input).unwrap();
            assert_eq!(result.to_string(), expected);
    ...
    fn expr_regex() {
        let tests = vec![
            // regex_literal
            ("kong.foo.foo5 ~ \"^foo.*$\"","(kong.foo.foo5 ~ \"^foo.*$\")"),
            // regex_literal
            ("kong.foo.foo6 ~ \"^foo.*$\"","(kong.foo.foo6 ~ \"^foo.*$\")"),
        ];
        for (input, expected) in tests {
            let result = parse(input).unwrap();
            assert_eq!(result.to_string(), expected);
    ...
    fn expr_transformations() {
        let tests = vec![
            // lower
            ("lower(kong.foo.foo13) == \"foo\"","(lower(kong.foo.foo13) == \"foo\")"),
            // any
            ("any(kong.foo.foo14) == \"foo\"","(any(kong.foo.foo14) == \"foo\")"),
        ];
        for (input, expected) in tests {
            let result = parse(input).unwrap();
            assert_eq!(result.to_string(), expected);
    ...
    fn expr_transformations_nested() {
        let tests = vec![
            // lower + lower
            ("lower(lower(kong.foo.foo15)) == \"foo\"", "(lower(lower(kong.foo.foo15)) == \"foo\")"),
            // lower + any
            ("lower(any(kong.foo.foo16)) == \"foo\"", "(lower(any(kong.foo.foo16)) == \"foo\")"),
            // any + lower
            ("any(lower(kong.foo.foo17)) == \"foo\"", "(any(lower(kong.foo.foo17)) == \"foo\")"),
            // any + any
            ("any(any(kong.foo.foo18)) == \"foo\"", "(any(any(kong.foo.foo18)) == \"foo\")"),
        ];
        for (input, expected) in tests {
            let result = parse(input).unwrap();
            assert_eq!(result.to_string(), expected);
