...
impl Matcher for RoutePattern {
    fn visit(&self, visitor: &mut MatcherVisitor) {
        visitor.visit_match_regex(&self.prefix);
    }
}

let routes = vec![
    RoutePattern { prefix: r"^/items/[\d*]$".to_string() },
    RoutePattern { prefix: r"^/users?/.*$".to_string() },
];
...
If we are able to find a small set of prefixes where one _must_ match at the begining of the actual string in order for the overall pattern to match, those found prefixes will be used as a prefilter.
